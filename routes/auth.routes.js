...
// it likes /api/auth/register
router.post('/resetpage',
    [
        check('password', 'Min length of password is 6 symbols').isLength({min: 6})
        check('macthPassword', 'Not equal of your password').isLength({min:6})
    ],
    async (req, res)=>{
        try{
            console.log('Body', req.body)
            ...
            const {password, macthPassword} = req.body
            console.log('start reset your password')
            if(macthPassword!==password){
                return res.status(400).json({message: 'Passwords do not match'})
            }
            console.log('password is ok')
            ...
            const password = new User({password: hashedPassword})
            await password.save()
            res.status(201).json({message: "Password re-created!"})
...
// it likes /api/auth/register
router.post('/forgotpage',
    [
        check('email', 'Insert email').exists()
    ],
    async (req, res)=>{
        try{
            console.log('Body:', req.body)
            ...
            if(!errors.isEmpty()){
                return res.status(400).json({
                    errors: errors.array(),
                    message: 'Incorrect data on email'
                })
            }
